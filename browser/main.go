...
//
// Webassemble code for answer SDP
package main
...
func main() {
	// Everything below is the Pion WebRTC API! Thanks for using i
	...
	var dataChannel *webrtc.DataChannel = nil
	...
	js.Global().Set("createQRCD", js.FuncOf(func(_ js.Value, _ []js.Value) interface{} {
...
func setImage() {
	el := getElementByID("message")
	msg := el.Get("value").String()

	fmt.Println("get " + msg)
	imgbase64 := qrcode.CreateBarcodeAsBase64(msg)
	fmt.Println("set " + imgbase64)
	qrcd := getElementByID("qrcode")
	qrcd.Set("src", imgbase64)
